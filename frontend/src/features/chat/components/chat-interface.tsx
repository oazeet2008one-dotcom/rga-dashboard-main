...
import { Brain, Sparkles, Plus, Mic, PenTool, TrendingUp, Lightbulb, FileText, Send, User, MessageSquare, Trash2, PanelLeftClose, PanelLeft, StopCircle, HelpCircle, Code, Shield } from "lucide-react";
...
export function ChatInterface({ isWidget = false }: { isWidget?: boolean }) {
    ...
    const handleSearch = async (q: string = query) => {
        ...
        setTimeout(() => {
            ...
            if (lowerQ.includes('help') || lowerQ.includes('support')) {
                responseText = "I can help you navigate the dashboard, interpret analytics, or configure settings. What specifically do you need assistance with today?";
            } else if (lowerQ.includes('bug') || lowerQ.includes('error')) {
                responseText = "Please describe the issue you're encountering. Including error messages or steps to reproduce helps me diagnose it faster. 🐛";
            } else if (lowerQ.includes('api') || lowerQ.includes('docs')) {
                responseText = "Here is the documentation link for our API: [API Docs](/docs). You can authenticate using your API Key from the Settings page.";
            } else {
                responseText = `I understand you're asking about "${q}". \n\nCould you provide a bit more context so I can give you the most accurate answer? I'm here to help with anything RGA.Data related!`;
            }
    ...
    const handleVoiceInput = () => {
        ...
        recognition.lang = 'en-US'; // Default to English for general chat, can be toggled
    ...
    return (
        <div className={cn(
            "w-full flex relative font-sans gap-0 transition-all duration-300 overflow-hidden",
            isWidget ? "h-full bg-slate-50" : "max-w-7xl mx-auto pt-6 h-[700px] md:gap-6 md:overflow-visible"
        )}>

            {/* Mobile Overlay Backdrop */}
            <AnimatePresence>
                {isSidebarOpen && !isWidget && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                    ...
                    <div className="flex-1 flex flex-col min-h-0 bg-slate-50/50 rounded-xl border border-slate-100/50 overflow-y-auto custom-scrollbar">
                        <div className="flex-1 px-2 py-2">
                            <div className="space-y-1">
                                {sessions.length > 0 && (
                                    <>
                                        <div className="px-2 py-1.5 text-xs font-semibold text-slate-400">History</div>
                        ...
                        {messages.length === 0 && (
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg">
                                    {[
                                        { label: 'Platform Help', icon: HelpCircle },
                                        { label: 'Technical Support', icon: Code },
                                        { label: 'Security & Access', icon: Shield },
                                        { label: 'General Inquiry', icon: FileText }
                                    ].map((action, index) => {
                ...
                <div className="p-4 bg-white/80 backdrop-blur-sm border-t border-slate-100 sticky bottom-0 z-10 transition-all">
                    <div className="relative group max-w-3xl mx-auto">
                        ...
                        <div className="text-center mt-2 text-[10px] text-slate-400/80 font-medium tracking-wide">
                            AI may produce inaccurate information. Please verify critical details.
                        </div>
                    </div>
                </div>
            </div>
